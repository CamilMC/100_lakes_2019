...
ggplot()+geom_sf(data=norgemap)+
  ...
  theme_void(base_size = 24)+labs(x="",y="")+
...
ggplot()+geom_sf(data=norgemap)+
  ...
  theme_void(base_size = 24)+labs(x="",y="")+
  scale_color_gradientn(colors = viridis(6,direction = -1,end=0.9),limits=c(0,3))+
  geom_text(data=filter(lakes70,Vmax.DOC >3),aes(x=long,y=lat,label=round(Vmax.LOC,2)),nudge_y = -0.09,nudge_x = -0.1)+  
  theme(legend.position = c(.9,.7))+
  xlim(5,15)+ylim(58,63)

dev.off()

# positively skewed parameters --> log -----
lakes70$logLOC <- log(lakes70$LOC)
lakes70$logVmax <- log(lakes70$Vmax)
lakes70$logT <- log(lakes70$T)
lakes70$logcond <- log(lakes70$cond_?S.m)
lakes70$logalk <- log(lakes70$alk_meq.L)
lakes70$logTOC <- log(lakes70$TOC)
lakes70$logTN <- log(lakes70$TN)
lakes70$logTP <- log(lakes70$TP)
lakes70$logDOC <- log(lakes70$DOC)
lakes70$logDN <- log(lakes70$DN)
lakes70$logDP <- log(lakes70$DP)
lakes70$logO2 <- log(lakes70$O2_?M)
lakes70$logN2 <- log(lakes70$N2_?M)
lakes70$logCO2 <- log(lakes70$CO2_?M)
lakes70$logCH4 <- log(lakes70$CH4_nM)
lakes70$logN2O <- log(lakes70$N2O_nM)
lakes70$logF <- log(lakes70$Fluoride_mg.L)
lakes70$logCl <- log(lakes70$Chloride_mg.L)
lakes70$logNO2 <- log(lakes70$Nitrite_mg.L)
lakes70$logNO3 <- log(lakes70$Nitrate_mg.L)
lakes70$logBr <- log(lakes70$Bromide_mg.L)
lakes70$logSO4 <- log(lakes70$Sulfate_mg.L)
lakes70$logNO3 <- log(lakes70$Nitrate_mg.L)
lakes70$logCr <- log(lakes70$Cr52)
lakes70$logNi <- log(lakes70$Ni58)
lakes70$logZn <- log(lakes70$Zn64)
lakes70$logAs <- log(lakes70$As75)
lakes70$logCd <- log(lakes70$Cd114)
lakes70$logPb <- log(lakes70$Pb208)
lakes70$logV <- log(lakes70$V51)
lakes70$logMn <- log(lakes70$Mn55)
lakes70$logCo <- log(lakes70$Co59)
lakes70$logCa <- log(lakes70$Ca)
lakes70$logMg <- log(lakes70$Mg)
lakes70$logNa <- log(lakes70$Na)
lakes70$logK <- log(lakes70$K)
lakes70$logFe <- log(lakes70$Fe)
lakes70$logAl <- log(lakes70$Al)
...
cor_param_Vmax.DOC <- c("Sample_ID","LOC","Vmax","s_275_295","SR","DN","Al_R","SUVA","DOC","Vmax.DOC")
cor_Vmax.DOC <- lakes70[cor_param_Vmax.DOC] %>% setNames(c("Sample_ID","LOC","Biodeg","s_275_295","SR","DN","Al","SUVA","DOC","Biodeg_DOC"))
...
csel <- c("Sample_ID","T","Vmax.LOC","Vmax","Vmax.DOC","tmax",
...
clust70c <- lakes70_clusters[,-which(names(lakes70_clusters) %in% c("X","Sample_ID","long","lat"))]
...
lakes70_scaled <- scale(sel70c) %>% as.data.frame()
...
#sel70$cluster <- cutree(dend70c,nb_clusters)
...
ptt$pvalue[which(ptt$pvalue < 0.05)] <- "significative"
ptt$pvalue[which(is.na(ptt$pvalue))] <- "non-significative"
...
x <- c("Biodeg","Biodeg_s","Biodeg_LOC", "SUVA","DN","Mg","K","As75",
       "DOC","Fe","Cu63","Ni58","V51","Co59","Temperature",
       "Al","cells_counts.mL","pH",
       "CH4","Altitude","Na",
