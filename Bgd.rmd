...
#sd_param <- sapply(lakes73log,sd, na.rm = T) %>% as.data.frame() %>% tibble::rownames_to_column() %>% setNames(c("param","sd"))
...
lasso_simple_plot <- function(lasso_list, title = ""){
  ...
  g1 <- qplot(w,x,size = 3)+
    labs(x = "Observed values", y = "Fitted values" , title = paste("Fitted values vs. observations",title, sep = " "))+
    geom_abline(intercept = 0, slope = 1, col = "gray")+
    annotate("label",x = max(w) - (max(w)-min(w))/6, y = min(x) + (max (x)-min(x))/6, label = paste("MAE",round(mae(w,x),2),sep = " = "), size = 5)+
    theme_bw(base_size=15)+ theme(panel.grid = element_line(color="gray95"), legend.position = "left")
  plot(g1)
...
#long_lasso <- merge(long_lasso,sd_param,by.x = "param", by.y = "param")


#long_lasso$param <- c("Cells","Cells","log(C:N)","log(C:N)","log(CO2)","log(DOC)","log(DOC)","log(DP)","log(DP)","log(DP)","log(EC)","log(Fe)","log(Fe)","log(Fe)","log(N2O)","log(O2)","pH","SARuv","SARuv","SARuv","SUVA","SUVA")
...
#long_lm <- merge(long_lm,sd_param,by.x = "lasso_param", by.y = "param")
...
long_lm %>% mutate(parameter = reorder_within(lasso_param,estimate,group)) %>%
  ...
  scale_y_reordered(position = "right")+
  facet_grid(rows = vars(group),scales = "free_y",switch = "y")+
  labs(x="",y="Multiple Linear Model Coefficients")+
  theme_light(base_size = 15)+
  theme(legend.position = "none",axis.text.x = element_text(angle = 45,hjust = 1),panel.grid.minor = element_line(colour = NA),panel.grid.major = element_line(colour = "gray95"))+
  scale_fill_viridis_d(end = 0.6)
